...
exports.activateAlarmController = async (req, res) => {
  ...
  try {
    ...
    if (!alarm) {
      return res.status(404).json({ error: `Тревога №${id} не найдена.` });
    }

    if (alarm.is_active) {
      return res.status(400).json({ error: `Тревога №${id} уже активировна.` });
    }
    ...
    res.status(500).json({ error: 'Ошибка отправления сигнала' });
  }
};