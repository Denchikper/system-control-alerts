...
export default function EditableDayColumn({ day, events, setLessonModalOpen, setDayChoicen, setEventsChoicen, handleDeleteLesson }) {
  return (
    <div className="bg-[#161B22] rounded-xl p-3 flex flex-col min-w-[200px] h-[450px] xl:h-[550px]"> 
      <div className="text-center text-sm text-gray-300 border border-gray-700 rounded-lg py-2 mb-2">
        {day.name}
      </div>

      <div className="flex-1 overflow-y-auto flex flex-col gap-2 pb-2 pr-1 custom-scrollbar-2">
        {events.length === 0 && (
        ...
        {events
          ...
          .map(event => (
            <EditableEventRow key={event.id} event={event} day={day} handleDeleteLesson={handleDeleteLesson} />
          ))}
      </div>
